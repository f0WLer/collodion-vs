...
namespace Collodion.Plates.Rendering
{
    internal static class PhotoMeshUtil
    {
        ...
        // Flags every quad in the mesh for the transparent render pass so it alpha-blends
        // (smooth gradient) instead of being alpha-tested into a hard cutout in the opaque pass.
        internal static void SetTransparentRenderPass(MeshData mesh)
        {
            if (mesh == null) return;
            int quadCount = mesh.VerticesCount / 4;
