...
namespace Photochemistry.ImageEffects
{
    ...
    internal static class ImageEffectsCommandPropertyMap
    {
        ...
        internal static bool TryApply(string property, string rawValue, ImageEffectsConfig cfg, out string? error)
        {
    ...
    {
        private readonly Action<ImageEffectsConfig, float>? _setFloat;
        private readonly Action<ImageEffectsConfig, int>? _setInt;
        private readonly Action<ImageEffectsConfig, bool>? _setBool;
        private readonly string? _label;

        // Creates a float-backed property binding.
        private ImageEffectsPropertyBinding(Action<ImageEffectsConfig, float> setter)
        {
            _setFloat = setter;
        }

        // Creates an int-backed property binding with a user-facing label for parse errors.
        private ImageEffectsPropertyBinding(string intLabel, Action<ImageEffectsConfig, int> setter)
        {
            _label = intLabel;
            _setInt = setter;
        }
        ...
        private ImageEffectsPropertyBinding(string boolLabel, Action<ImageEffectsConfig, bool> setter)
        {
            _label = boolLabel;
            _setBool = setter;
        }
        ...
        internal static ImageEffectsPropertyBinding ForFloat(Action<ImageEffectsConfig, float> setter)
        {
            return new ImageEffectsPropertyBinding(setter);
        }

        // Builds a binding for int properties.
        internal static ImageEffectsPropertyBinding ForInt(string label, Action<ImageEffectsConfig, int> setter)
        {
            return new ImageEffectsPropertyBinding(label, setter);
        }

        // Builds a binding for bool properties.
        internal static ImageEffectsPropertyBinding ForBool(string label, Action<ImageEffectsConfig, bool> setter)
        {
            return new ImageEffectsPropertyBinding(label, setter);
        }

        // Parses and applies one raw command value through the active typed setter.
        internal bool TryApply(ImageEffectsConfig cfg, string rawValue, out string? error)
        {
            if (_setFloat != null)
            {
