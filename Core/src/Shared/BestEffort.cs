...
namespace Photocore
{
    ...
    {
        private static readonly Dictionary<string, long> _lastWarningMsByOperation = new(StringComparer.Ordinal);
        private static readonly object _warningGate = new();
        ...
        private static void LogFailure(ILogger? logger, string operation, Exception ex, BestEffortLogPolicy policy)
        {
            ...
            lock (_warningGate)
            {
                if (!_lastWarningMsByOperation.TryGetValue(operation, out long lastWarnMs) || nowMs - lastWarnMs >= policy.WarningIntervalMs)
                {
                    _lastWarningMsByOperation[operation] = nowMs;
                    shouldWarn = true;
                }
                else
                {
