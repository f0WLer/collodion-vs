    ...
    internal sealed class VirtualCameraPreviewRenderer : IRenderer, IDisposable, IExposurePreviewSink
    {
        ...
        public void OnRenderFrame(float deltaTime, EnumRenderStage stage)
        {
            ...
            try
            {
                ...
                // Run the idle frame through the GPU emulsion pipeline.
                VirtualExposureRenderer er = ExposureRenderer;
                int srcW = camera.fbo.Width;
                int srcH = camera.fbo.Height;
                GpuExposureAccumulator.ComputeTargetDimensions(srcW, srcH, cfg.DebugPreviewMaxDimension, out int outW, out int outH);

                using var accum = new GpuExposureAccumulator(_capi, outW, outH, referenceFrameCount: 1);
                accum.NormalizeByActualFrameCount = true;
                var p = er.ActiveProcess;
                accum.LinearizeInput             = er.Physics.Linearize;
                accum.ApplySpectralWeights        = er.Physics.SpectralWeights;
                accum.ApplyHDCurve               = er.Physics.HDCurve;
                accum.RedSensitivity             = er.Physics.EffectiveRedSens(p);
                accum.GreenSensitivity           = er.Physics.EffectiveGreenSens(p);
                accum.BlueSensitivity            = er.Physics.EffectiveBlueSens(p);
                ...
                if (_previewPixelScratch == null || _previewPixelScratch.Length < count)
                    _previewPixelScratch = new int[count];
                Marshal.Copy(cropped.GetPixels(), _previewPixelScratch, 0, count);
                _latestPreviewPixels = _previewPixelScratch;
                _latestPreviewWidth  = cropped.Width;
                _latestPreviewHeight = cropped.Height;
                if (!ReferenceEquals(cropped, developed)) cropped.Dispose();
            }
            catch (Exception ex)
            {
