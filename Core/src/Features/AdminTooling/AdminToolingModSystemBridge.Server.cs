...
namespace Photocore.AdminTooling
{
    ...
    internal sealed partial class AdminToolingModSystemBridge
    {
        ...
        internal AdminToolingModSystemBridge(PhotocoreModSystem owner)
        {
            _owner = owner;
        }
        ...
        internal void ConfigureServerOperatorToolingStartup(ICoreServerAPI api)
        {
            ...
            BestEffort.Try(_owner.BestEffortLogger,
                ...
                () => api.Logger.Notification($"photocore: loaded config '{PhotocoreModSystem.ConfigFileName}'"));
            ...
            api.Network.GetChannel("photocore")
        ...
        private void OnGiveSensitizedPlateReceived(IServerPlayer player, GiveSensitizedPlatePacket packet)
        {
            ...
            AssetLocation itemCode = recipe?.SensitizedItemCode ?? new AssetLocation("photocore", "sensitizedplate");
            ...
            if (recipe?.Substrate is null or "glass")
                PlateAttributes.SetNameLangCode(stack, "photocore:plate-name-sensitized");
