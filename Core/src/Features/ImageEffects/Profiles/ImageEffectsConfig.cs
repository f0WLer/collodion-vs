...
namespace Photochemistry.ImageEffects
{
    ...
    public sealed class ImageEffectsConfig
    {
        ...
        // Realism tuning (0..1). Keep subtle; these break the "perfect filter" feel.
        // Imperfection: biases dust toward edges and adds slight one-sided density pooling.
        public float Imperfection = 0.60f;

        // 0..1: makes the plate edges slightly warmer / more sepia than the centre (uneven toning / oxidation).
        // A spatial aging artifact — distinct from the uniform image colour, which the per-chemistry silver tone owns.
        ...
        // Fraction (0..1) of the image height treated as "sky/top area" for SkyUnevenness and SkyBlowout.
        ...
        // 0..1 sky blowout/bloom strength (spatial bloom around blown highlights, applied mainly to the top).
        ...
        // Edge-warmth internals (the width of the warm border band and its blend scale).
        public float EdgeWarmthWidthMinPx = 4.0f;
        public float EdgeWarmthWidthFraction = 0.18f;
        ...
        // DynamicScale is a +/- percentage (0.05 => +/-5%) applied to select artifact parameters.
        ...
        // Clamps edge-warmth border behaviour.
        private void ClampEdgeWarmthSettings()
        {
            EdgeWarmthWidthMinPx = ClampRange(EdgeWarmthWidthMinPx, 0f, 256f);
            EdgeWarmthWidthFraction = ClampRange(EdgeWarmthWidthFraction, 0f, 2f);
        ...
        public ImageEffectsConfig Clone()
        {
            return (ImageEffectsConfig)MemberwiseClone();
        }
    }
}
